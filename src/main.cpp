...
void fill_up_boxes(part_sorting_list &tempPSL)
{
	...
	if (tempPSL.current_box !=
		NULL)
	{ // Makes sure there is a box to sort parts into
		if (tempPSL.current_box->no_free_boxes > 0 &&
			tempPSL.number_parts_left >
				0)
		{ // check if there any parts and boxes left to sort into
			unsigned int possible_parts =
				tempPSL.number_parts_left / tempPSL.current_box->no_items_perbox;
			if (possible_parts >
				0)
			{ // If an integral number of parts could be sorted then, proceed
		...
		{ // If there is no currentbox, this impliess that we have exhuasted
