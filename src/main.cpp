...
bool positive_int_check(std::string &);

/**
 * @brief Checks whether the given string contains only positive integers.
 * Whitspaces are ignored
 *
 * @param in_string
 * @return true implies that the given string contains only digits
 * @return false implies that the given string contains more than just digits
 */
bool positive_int_check(std::string &in_string) {
  for (char &c : in_string) {
    if (!isdigit(c) && c != ' ') // Checks if each char in the provided string
                                 // is either a digit or space
    {
...
void get_total_parts(part_sorting_list &tempPSL) {
  ...
  } while (!positive_int_check(
      input)); // While the input does not contain only poistive integers
  std::string::size_type sz;
  try { // check if the numbers can be extracted from the string, if the number
        // of inputs are lesser than require then it throws an error and tries
        // again
    tempPSL.number_parts_left = std::stoi(input, &sz);
  } catch (const std::exception &e) {
    std::cerr << "Insufficient inputs/bad input\n";
    get_total_parts(tempPSL);
  }
...
void get_total_boxes(part_sorting_list &tempPSL) {
  ...
  do {
    ...
    std::getline(std::cin, input);
  } while (!positive_int_check(input));
  std::string::size_type sz;
  try { // check if the numbers can be extracted from the string, if the number
        // of inputs are lesser than require then it throws an error and tries
        // again
    tempPSL.box_S.no_boxes = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_M.no_boxes = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_L.no_boxes = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_XL.no_boxes = std::stoi(input, &sz);
  } catch (const std::exception &e) {
    std::cerr << "Insufficient inputs/bad input" << '\n';
    get_total_boxes(tempPSL);
  }
...
void get_parts_per_box(part_sorting_list &tempPSL) {
  ...
  try { // check if the numbers can be extracted from the string, if the number
        // of inputs are lesser than require then it throws an error and tries
        // again
    tempPSL.box_S.no_items_perbox = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_M.no_items_perbox = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_L.no_items_perbox = std::stoi(input, &sz);
    input = input.substr(sz, input.length());
    tempPSL.box_XL.no_items_perbox = std::stoi(input, &sz);
  } catch (const std::exception &e) {
    std::cout << " Insufficient inputs/bad input\n";
    get_parts_per_box(tempPSL);
  }
  if (!(tempPSL.box_S.no_items_perbox < tempPSL.box_M.no_items_perbox &&
        tempPSL.box_M.no_items_perbox < tempPSL.box_L.no_items_perbox &&
        tempPSL.box_L.no_items_perbox <
            tempPSL.box_XL
                .no_items_perbox)) { // Make sure that the items per box of
                                     // smaller boxes are lesser than those of
                                     // the bigger ones
    std::cout << "Input does not follow rule \n";
    get_parts_per_box(tempPSL);
  }
}
